...
INSTRUCTION="今日日期必须使用：${TODAY}（${TODAY_CN}），文件名与标题中的日期一律为此日，不得使用 2025 年或其他年份。严格按照 @${COMMAND_FILE} 的完整流程执行：按优先级做多轮实时搜索（Web Search），筛选排序后按该文件中的 Markdown 模板整理，最后将日报写入 NEWS/science/ 对应日期的 md 文件。不要省略任何步骤。"
